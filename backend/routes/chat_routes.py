...
3. 优先依据文档内容回答。"""
            ...
            if not image_list and not use_agent and web_search_allowed_for_inventory:
                ...
                if _do_web_search:
                    ...
                    yield f"data: {json.dumps({'type': 'web_search_status', 'phase': 'fetch_complete', 'count': len(web_search_sources), 'query': web_search_query or _web_search_query_meta_for_stream.get('effective_query', '') or _web_search_display_query, 'audit': web_search_audit}, ensure_ascii=False)}\n\n"
                else:
                    _finalize_unattempted_web_search_audit(
                        web_search_audit,
                        reason="auto_policy_not_selected",
