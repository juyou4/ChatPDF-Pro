...
包含 JSON KV 存储、NanoVectorDB 向量存储、NetworkX 图存储的具体实现。
"""
...
class JsonKVStorage(BaseKVStorage):
    ...
    def __post_init__(self):
        ...
        logger.info(f"[GraphRAG] 加载 KV {self.namespace}: {len(self._data)} 条")
...
class NanoVectorDBStorage(BaseVectorStorage):
    ...
    async def upsert(self, data: dict[str, dict]):
        logger.info(f"[GraphRAG] 插入 {len(data)} 向量到 {self.namespace}")
        if not len(data):
            logger.warning("[GraphRAG] 插入空数据到向量库")
            return []
        list_data = [
            {
                "__id__": k,
...
class NetworkXStorage(BaseGraphStorage):
    ...
    def write_nx_graph(graph: nx.Graph, file_name):
        logger.info(
            f"[GraphRAG] 写入图: {graph.number_of_nodes()} 节点, {graph.number_of_edges()} 边"
        )
    ...
    def __post_init__(self):
        ...
        if preloaded_graph is not None:
            logger.info(
                f"[GraphRAG] 从文件加载图: {preloaded_graph.number_of_nodes()} 节点, "
                f"{preloaded_graph.number_of_edges()} 边"
            )
    ...
    async def clustering(self, algorithm: str):
        if algorithm not in self._clustering_algorithms:
            raise ValueError(f"聚类算法 {algorithm} 不受支持")
    ...
    async def community_schema(self) -> dict[str, SingleCommunitySchema]:
        ...
        for node_id, node_data in self._graph.nodes(data=True):
            ...
            for cluster in clusters:
                ...
                results[cluster_key]["chunk_ids"].update(
                    node_data.get("source_id", "").split(GRAPH_FIELD_SEP)
                )
        ...
        for k, v in results.items():
            ...
            v["occurrence"] = len(v["chunk_ids"]) / max(max_num_ids, 1)
        return dict(results)
    ...
    async def _leiden_clustering(self):
        ...
        logger.info(f"[GraphRAG] 各层级社区数: {dict(__levels)}")
