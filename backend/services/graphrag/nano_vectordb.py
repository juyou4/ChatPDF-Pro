...
基于 numpy 的内存向量数据库，支持 JSON 文件持久化。
参考 https://github.com/gusye1234/nano-vectordb
"""
...
def load_storage(file_name) -> DataBase | None:
    ...
    logger.info(f"[NanoVectorDB] 加载 {data['matrix'].shape} 数据")
    return data
...
class NanoVectorDB:
    ...
    def __post_init__(self):
        ...
        self.pre_process()
        logger.info(f"[NanoVectorDB] Init {asdict(self)} {len(self.__storage['data'])} data")
