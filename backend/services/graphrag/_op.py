...
包含文本分块、实体提取（含 gleaning）、社区报告生成、Local Query 等核心逻辑。
"""
...
def chunking_by_token_size(
        content: str, overlap_token_size=128, max_token_size=1024, tiktoken_model="gpt-4"
):
    """按 token 大小分块"""
...
async def _handle_entity_relation_summary(
    ...
    logger.debug(f"[GraphRAG] 触发实体摘要: {entity_or_relation_name}")
...
async def extract_entities(
    ...
    logger.info(f"[GraphRAG] 从 {len(ordered_chunks)} 个块中提取实体")
    ...
    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        ...
        logger.debug(
            f"[GraphRAG] {now_ticks} 已处理 {already_processed} 块, "
            f"{already_entities} 实体(含重复), {already_relations} 关系(含重复)"
        )
    ...
    if not len(all_entities_data):
        logger.warning("[GraphRAG] 未提取到任何实体，请检查 LLM 是否正常工作")
        return None
...
async def _pack_single_community_describe(
    ...
    truncated = len(nodes_list_data) > len(nodes_may_truncate_list_data) or len(
        edges_list_data) > len(edges_may_truncate_list_data)

    report_describe = ""
    if truncated and len(community["sub_communities"]) and len(already_reports):
        logger.info(
            f"[GraphRAG] 社区 {community['title']} 超出限制，使用子社区报告"
        )
        report_describe, report_size, contain_nodes, contain_edges = _pack_single_community_by_sub_communities(
...
async def generate_community_report(
    ...
    async def _form_single_community_report(
        ...
        already_processed += 1
        now_ticks = PROMPTS["process_tickers"][already_processed % len(PROMPTS["process_tickers"])]
        logger.debug(f"[GraphRAG] {now_ticks} 已生成 {already_processed} 个社区报告")
        return data

    levels = sorted(set([c["level"] for c in community_values]), reverse=True)
    logger.info(f"[GraphRAG] 按层级生成报告: {levels}")
    community_datas = {}
    for level in levels:
...
async def _find_most_related_community_from_entities(
    ...
    related_community_keys = sorted(
        ...
        key=lambda k: (
            ...
            related_community_datas.get(k, {}).get("report_json", {}).get("rating", -1),
        ),
        reverse=True,
    )
...
async def _find_most_related_text_unit_from_entities(
    ...
    if any([v is None for v in all_text_units_lookup.values()]):
        logger.warning("[GraphRAG] 部分文本块缺失，存储可能损坏")
    all_text_units = [
...
async def _build_local_query_context(
    ...
    if not all([n is not None for n in node_datas]):
        logger.warning("[GraphRAG] 部分节点缺失，存储可能损坏")
    ...
    logger.info(
        f"[GraphRAG] 使用 {len(node_datas)} 实体, {len(use_communities)} 社区,"
        f" {len(use_relations)} 关系, {len(use_text_units)} 文本块"
    )
...
async def local_query(
    ...
    """执行 Local Query：实体+关系+社区报告+源文本块"""
    ...
    logger.debug(f"[GraphRAG] query: {query}, prompt len: {len(sys_prompt)}")
